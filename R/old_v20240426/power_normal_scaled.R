#' Power-law fit with scaled normally distributed residuals
#'
#' @md
#' @description
...
#'   sigma = sigma0*x^beta
#'
#' where sigma0 is the power-law multiplier for the standard deviation.
#'
#' The optimal fitting parameters y0, beta and sigma0 are found by maximising
...
#'   * `sd_exponent`: assumed standard deviation power-law exponent (beta)
...
power_normal_scaled_fit <- function(
  ...
  ks <- stats::ks.test(y/(x^beta), "pnorm", y0, sigma0)
  # return list
  list(
    loglikelihood = logL,
    multiplier = y0,
...
#' Calculate power-law normal (scaled) loglikelihood
#'
#' @description
...
#'
#' @inheritParams power_normal_scaled_fit
#' @param par vector with fitting parameters (power-law multiplier, power-law
#'   exponent, and standard deviation power-law multiplier)
...
power_normal_scaled_loglikelihood <- function(
  ...
  # loglikelihood
  if (deriv == 0) {
    -c1*(log(sigma0) + 0.5*log(2*pi)) - beta*c2 -
      (c6 - 2*y0*c3 + c1*y0^2)/(2*sigma0^2)
  } else if (deriv == 1) {
    c(
      (c3 - c1*y0)/sigma0^2,
      (c7 - y0*c4)/sigma0^2 - c2,
      (c1*y0^2 - 2*c3*y0 + c6)/sigma0^3 - c1/sigma0
    )
  } else if (deriv == 2) {
    d2logL_dy02 <- -c1/sigma0^2
    d2logL_dy0dbeta <- -c4/sigma0^2
    d2logL_dy0dsigma0 <- 2*(c1*y0 - c3)/sigma0^3
    d2logL_dbeta2 <- (y0*c5 - 2*c8)/sigma0^2
    d2logL_dbetadsigma0 <- 2*(y0*c4 - c7)/sigma0^3
    d2logL_dsigma02 <- c1/sigma0^2 - 3*(c1*y0^2 - 2*c3*y0 + c6)/sigma0^4
    matrix(
      c(
...
#' Root solve exponent in normal (scaled) power-law
#'
#' @description
#' Root solve equation to obtain the exponent in power-law fitting
#' with normal (scaled)-distribution
#'
#' @inheritParams power_normal_scaled_fit
...
power_normal_scaled_root <- function(
  ...
  c7 <- sum(weights*x^(-2*beta)*y^2*log(x))
  # root
  c1*(c1*c7 - c3*c4)/(c1*c6 - c3^2) - c2
}


#' Jacobian of function `power_normal_scaled_root()`
#'
#' @description
#' Returns the derivative of the function `power_normal_scaled_root()`
#' with respect to input argument `beta`
#'
#' @inheritParams power_normal_scaled_root
#' @return derivative of `power_normal_scaled_root()` with respect to `beta`
#' @examples
#' # define parameters
#' y0 <- 20
#' beta <- -0.5
...
power_normal_scaled_root_jacobian <- function(
  ...
  c8 <- sum(weights*x^(-2*beta)*y^2*log(x)^2)
  # jacobian of root
  c1*(-2*c1*c8 + c4^2 + c3*c5)/(c1*c6 - c3^2) +
    2*c1*(c1*c7 - c3*c4)^2/(c1*c6 - c3^2)^2
}
