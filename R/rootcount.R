...
#'
#' @param dfp dataframe with fitting parameters per power-law segment. This
...
#'
rootcount_cumulative_fitted <- function(dfp, n = 101) {
  # start and end values
  dfp$bundle_id <- seq(1, nrow(dfp))
  dfp$ymax <- cumsum(dfp$total)
  dfp$ymin <- c(0, dfp$ymax)[1:nrow(dfp)]
  # get all x-coordinates
  df <- expand.grid.df(data.frame(s = seq(0, 1, l = n)), dfp)
...
#' plot(df$x, df$y, "l")
#'
rootcount_density_fitted <- function(dfp, n = 101) {
  # assign bundle id
  dfp$bundle_id <- seq(1, nrow(dfp))
  # get multiplication coefficients
  dfp$alpha <- with(dfp, ifelse(
    is_near(power, -1),
    total/log(xmax/xmin),
