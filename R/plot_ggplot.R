...
#' @param df dataframe with fitting values per segment. For more information,
...
rootcount_cumulative_plot <- function(
  ...
  df2 <- rootcount_cumulative_fitted(df, n = n)
  # axes limits
  xlims <- round_range(c(x, df$xmin), lim = xlim, ticks = ticks)
...
rootcount_density_plot <- function(
  ...
  df2 <- rootcount_density_fitted(df, n = n)
  df2 <- data.frame(
